...
writeSegmentTable = function(sgs, title, subtitle="", fn, sortBy, sortDecreasing=FALSE,
  colors=c("#d0d0d0", "#ffffff"), interact, HTML=TRUE, open="wt",
  ## column ordering
  colOrd = c("segID", "category", "overlap", "overlappingFeature", "oppositeFeature",
    "chr", "strand", "start", "end", "length", "level", "utr5", "utr3",
    "z3", "z5", "featureInSegment", "mostOfFeatureInSegment", "overlapFeatAll", 
  ...
  for(cn in intersect(colnames(sgs), c("featureInSegment", "mostOfFeatureInSegment", "overlappingFeature", "oppositeFeature")))
  ...
  if(!missing(sortBy)){
    rowOrd = switch(sortBy,
      ...
      "level" = {
        stopifnot("level" %in% names(sgs))
        order(sgs[, "level"], decreasing=TRUE)
      },
      "goodUTR" = {
