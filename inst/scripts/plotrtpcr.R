...
rnaTypes = c("seg-tot-050909", "seg-polyA-050909", "seg-odT-050909")[c(1,3)]
# 1. seg-tot-x is total-cDNA, while Lior's total-RH6 seems to refer to the total-RNA-RH6 samples;
# 2. Lior's poly-A samples seem to refer to our oligo-dT samples
...
if(!exists("rtpcrRes")) {
  ...
  rtpcrRes$array2 <- numeric(nrow(rtpcrRes))
  ...
  for(j in 1:nrow(rtpcrRes)) {
    ...
    myFun = function(dat) {
      ...
      median(dat$y[sel, ]) # note that this averages over replicates and probes
    }

    rtpcrRes$array1[j] = - get("theThreshold", get(rnaTypes[1])) + #0.5(
      myFun(get(paste(chr, geneStrand[gene[j]], "dat", sep="."), get(rnaTypes[1])))# +
    rtpcrRes$array2[j] = - get("theThreshold", get(rnaTypes[2])) + #0.5(
      myFun(get(paste(chr, geneStrand[gene[j]], "dat", sep="."), get(rnaTypes[2])))# +
  }#for(j in 1:nrow(rtpcrRes))
}
...
if (doPlot){
  ...
  myPlot = function(x, y, fac, txtLabel,mypch=16, ...){
    stopifnot(length(x)==nrow(y), length(x)==length(fac))
    stopifnot(nlevels(fac)==length(colors))
    mycolors <- colors[as.integer(fac)]
    if (ncol(y)>1){
      cat(ncol(y),"samples...\n")
      plot(x, y[,1], col=mycolors, pch=mypch, ...)
      for (j in 2:ncol(y))
        points(x,y[,j], col=mycolors, pch=mypch)
    } else {
  ...
  myPlot(rtpcrRes$array1, -apply(rtpcrRes[,c("tot..RH6_1","tot..RH6_2","tot..RH6_3")],2,median), gene, region,mypch=16, xlab="Array Expression (total-RNA)", ylab="rtPCR (total-RNA-RH6) [Ct]")
