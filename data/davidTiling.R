...
## Taken from inst/scripts/make.R, but modified to read the compressed CEL files directly.
## --------------------------------------------------

# This should be safe because tilingArray is in Depends
library("tilingArray")

celDir <- "../inst/celfiles"

adfData <- data.frame(
  filename = I(c(
    "09_11_04_S96_genDNA_16hrs_45C_noDMSO.cel.gz",
...
davidTiling <- tilingArray::readCel2eSet(
  adf = pd,
  path = celDir,
  rotated = TRUE,
  experimentData = ed
